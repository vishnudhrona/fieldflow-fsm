  ...
  return (
    <>
      {mode === 'badge' ? (
        <div className='mb-2'>
          <span
            className={`inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider border transition-all duration-300 ${
              isOnline
        ...
        <div className='flex items-center bg-slate-50 border border-slate-200/80 rounded-full shadow-sm'>
          {MODES.map((m) => {
            const Icon = m.icon;
            ...
            return (
              <Button
                ...
                title={m.label}
                className={`p-1.5 sm:px-3 sm:py-1 text-xs rounded-full border transition-all duration-200 shadow-none hover:bg-transparent min-w-0 ${
                  isActive
                    ? m.activeClass
                    : isDisabled
