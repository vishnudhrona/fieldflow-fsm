    ...
    >
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            <span>{tab.label}</span>
            {tab.count !== undefined && tab.count !== null && <span className='font-bold'>({tab.count})</span>}

            {isActive && <span className='absolute bottom-0 left-0 right-0 h-0.5 bg-[#D12026] rounded-full' />}
          </button>
        );
      })}
    </div>
