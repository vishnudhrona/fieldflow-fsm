    ...
    >
      <div className='p-3.5 flex items-center justify-between gap-3'>
        <div className='flex items-center gap-3 min-w-0 flex-1'>
          ...
          <div className='min-w-0 flex-1'>
            <h3 className='font-bold text-slate-900 text-sm tracking-tight truncate leading-snug'>
              {title}
            </h3>
      ...
      {(leftStat !== undefined || rightStat !== undefined) && (
        <div className='border-t border-slate-100 bg-slate-50/40 grid grid-cols-2 divide-x divide-slate-100 text-xs'>
          <div className='py-2 px-3 text-center truncate font-medium text-slate-600'>
            {leftStat ?? '—'}
          </div>
          <div className='py-2 px-3 text-center truncate font-bold text-[#D12026]'>
            {rightStat ?? '—'}
          </div>
        </div>
      )}
